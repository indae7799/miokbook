...
function getStatusUpdate(itemStatus) {
    if (!itemStatus)
        return null;
    if (OUT_OF_PRINT_STATUSES.includes(itemStatus))
        return 'out_of_print';
    if (itemStatus === OLD_EDITION_STATUS)
        return 'old_edition';
    return null;
}
exports.syncBookStatus = (0, scheduler_1.onSchedule)({ schedule: '0 2 * * *', timeZone: 'Asia/Seoul' }, async () => {
