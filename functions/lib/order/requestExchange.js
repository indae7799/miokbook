...
exports.requestExchange = void 0;
/**
...
function isWithinExchangePeriod(deliveredAt) {
    if (!deliveredAt)
        return false;
...
exports.requestExchange = (0, https_1.onCall)({ enforceAppCheck: false }, async (request) => {
    ...
    if (status === 'exchange_completed')
        throw new https_1.HttpsError('failed-precondition', 'EXCHANGE_ALREADY_COMPLETED');
    if (status.startsWith('return_'))
        throw new https_1.HttpsError('failed-precondition', 'ALREADY_RETURN_PROCESS');
    ...
    if (!isWithinExchangePeriod(deliveredAt))
        throw new https_1.HttpsError('failed-precondition', 'EXCHANGE_PERIOD_EXPIRED');
