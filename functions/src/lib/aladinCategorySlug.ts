...
export function mapAladinCategoryToSlug(categoryName: string | undefined): string {
  ...
  if (
    /컴퓨터|인터넷|it|프로그래밍|개발|모바일|소프트웨어|데이터|인공지능|ai|네트워크|보안|클라우드/.test(
      lower,
    )
  )
    return 'IT';
  ...
  // 경제/경영
  if (/경제|경영|투자|재테크|주식|부동산|창업|마케팅|회계|세금|재무/.test(raw)) return '경제';

  // 소설/문학/만화
  if (
    /소설|희곡|시\/희곡|문학|장르소설|판타지|로맨스|무협|추리|sf|만화|웹툰|라이트노벨/.test(raw)
  )
    return '소설';
  ...
  // 인문
  if (
    /인문|역사|철학|종교|심리학|정치|사회학|사회과학|법|교육|자기계발|리더십|성공/.test(raw)
  )
    return '인문';

  // 기타: 실용/생활/어린이/학습 등
  if (
    /가정|요리|육아|건강|의학|운동|스포츠|취미|여행|요가|다이어트/.test(raw) ||
