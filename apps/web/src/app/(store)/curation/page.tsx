...
export default async function CurationPage() {
  let cms: CmsHomeData = { featuredBooks: [], themeCurations: [] };
  try {
    cms = await getCmsHome();
  } catch {
    // 500 방지
  }
  const { featuredBooks, themeCurations } = cms;

  return (
    <main className="min-h-screen py-6 space-y-8 max-w-[1200px] mx-auto px-4">
      ...
      {featuredBooks.length === 0 && themeCurations.length === 0 && (
