...
function buildNewBooksQuery(category?: string, pageNum?: number): string {
...
function goNewBooks(router: ReturnType<typeof useRouter>, category?: string, pageNum = 1) {
  const queryString = buildNewBooksQuery(category, pageNum);
  router.replace(queryString ? `/new-books?${queryString}` : '/new-books');
}
...
function NewBooksPageClientInner({ books }: { books: BookCardBook[] }) {
  ...
  useEffect(() => {
    if (pageRaw > totalPages) {
      goNewBooks(router, category || undefined, totalPages);
    }
  ...
  const selectCategory = (nextCategory?: string) => {
    goNewBooks(router, nextCategory, 1);
    ...
    <>
      <main className="mx-auto min-h-screen max-w-[1280px] px-4 pb-2 pt-6">
        <div className="mb-6 flex flex-col gap-2 sm:flex-row sm:items-end sm:justify-between">
          <div>
            <h1 className="text-2xl font-semibold tracking-tight">금주의 출간된 새 책들</h1>
            <p className="mt-1 text-sm text-muted-foreground">
              최근 등록된 새 책을 카테고리별로 둘러보고 페이지 단위로 탐색할 수 있습니다.
            </p>
          </div>

          <div className="flex items-center gap-3">
            <p className="text-sm text-muted-foreground">
              총 <span className="font-semibold text-primary">{filteredBooks.length.toLocaleString()}</span>권
            </p>
        ...
        <div className="grid gap-6 lg:grid-cols-[240px_minmax(0,1fr)]">
          ...
          <section className="min-w-0">
                ...
                <div className="flex items-center">
                  ...
                  {totalPages > 1 ? (
                    <nav className="ml-auto hidden shrink-0 items-center gap-0.5 pb-px sm:flex" aria-label="페이지 이동">
                      <Button variant="ghost" size="icon" className="size-8" disabled={!hasPrev} onClick={() => goNewBooks(router, category || undefined, page - 1)}>
                      ...
                      {pageNumbers.map((pageNumber, index) =>
                        pageNumber === 'ellipsis' ? (
                          ...
                          <Button
                            ...
                            onClick={() => pageNumber !== page && goNewBooks(router, category || undefined, pageNumber)}
                          >
                            {pageNumber}
                          </Button>
                        ),
                      )}
                      <Button variant="ghost" size="icon" className="size-8" disabled={!hasNext} onClick={() => goNewBooks(router, category || undefined, page + 1)}>
            ...
            {pagedBooks.length === 0 ? (
              <p className="py-16 text-center text-sm text-muted-foreground">표시할 새 책이 없습니다.</p>
            ) : (
            ...
            {totalPages > 1 ? (
              <nav className="flex items-center justify-center gap-1 pb-2 pt-6 sm:hidden" aria-label="페이지 이동">
                <Button variant="ghost" size="icon" className="size-9" disabled={!hasPrev} onClick={() => goNewBooks(router, category || undefined, page - 1)}>
                  <ChevronLeft className="size-4" />
                </Button>
                {pageNumbers.map((pageNumber, index) =>
                  pageNumber === 'ellipsis' ? (
                ...
                <Button variant="ghost" size="icon" className="size-9" disabled={!hasNext} onClick={() => goNewBooks(router, category || undefined, page + 1)}>
                  <ChevronRight className="size-4" />
                </Button>
              </nav>
            ) : null}
...
export default function NewBooksPageClient({ books }: { books: BookCardBook[] }) {
  return (
    <Suspense fallback={null}>
      <NewBooksPageClientInner books={books} />
    </Suspense>
  );
}
