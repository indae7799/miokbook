...
export default function AdminOrdersPage() {
  ...
  const handleExportCsv = async () => {
    ...
    try {
      ...
      if (isTruncated) {
        toast.warning(`최대 ${Number(maxRows).toLocaleString('ko-KR')}건까지만 내려받았습니다. 기간을 좁혀 다시 시도해 주세요.`);
      } else {
        toast.success('CSV 다운로드가 완료되었습니다.');
      }
    } catch (exportError) {
      toast.error(exportError instanceof Error ? exportError.message : '다운로드에 실패했습니다.');
    } finally {
      setExporting(false);
    }
  };
  ...
  const handleDeleteRange = async () => {
    if (!user) return;
    if (!dateFrom || !dateTo) {
      toast.error('삭제 기간을 먼저 선택해 주세요.');
      return;
    }

    const label = statusFilter ? (STATUS_LABELS[statusFilter] ?? statusFilter) : '전체 주문';
    if (!window.confirm(`${dateFrom} ~ ${dateTo} 기간의 ${label} 데이터를 DB에서 삭제합니다. 계속할까요?`)) {
      return;
    }
    ...
    try {
      ...
      await queryClient.invalidateQueries({ queryKey: ['admin', 'orders'] });
      toast.success(`${Number(data.deletedCount ?? 0).toLocaleString('ko-KR')}건 삭제했습니다.`);
    } catch (deleteError) {
      toast.error(deleteError instanceof Error ? deleteError.message : '주문 삭제에 실패했습니다.');
    } finally {
      setDeleting(false);
    }
  };
  ...
  if (error) {
    return (
      <EmptyState title="주문 목록을 불러올 수 없습니다" message={error instanceof Error ? error.message : '오류가 발생했습니다.'} />
    );
  }
  ...
  return (
    <main className="space-y-5">
      <div className="border-b border-border pb-4">
        ...
        <p className="mt-2 text-sm leading-6 text-muted-foreground">주문 상태, 배송 처리, 반품/교환 요청, 프로모션 적용 내역을 한 화면에서 관리합니다.</p>
      </div>
      ...
      <section className="border border-border bg-background p-4">
        <div className="flex flex-col gap-3 lg:flex-row lg:items-center">
          ...
          <div className="lg:ml-auto flex gap-2">
            ...
            <Button variant="destructive" size="sm" onClick={handleDeleteRange} disabled={deleting}>
              {deleting ? '삭제 중...' : '기간 삭제'}
            </Button>
          </div>
        </div>
              ...
              <tbody>
                {returnRequested.map((row) => (
                  <tr key={row.id} className="border-b border-red-50">
                    <td className="px-3 py-3 font-mono text-xs">{row.displayOrderId ?? row.orderId}</td>
                    <td className="px-3 py-3 whitespace-pre-wrap">{row.returnReason || '-'}</td>
              ...
              <tbody>
                {exchangeRequested.map((row) => (
                  <tr key={row.id} className="border-b border-blue-50">
                    <td className="px-3 py-3 font-mono text-xs">{row.displayOrderId ?? row.orderId}</td>
                    <td className="px-3 py-3 whitespace-pre-wrap">{row.exchangeReason || '-'}</td>
                    <td className="px-3 py-3">
                      <Button
                        size="sm"
        ...
        <div className="overflow-x-auto">
          <table className="w-full min-w-[980px] text-sm">
            ...
            <tbody>
              {orders.length === 0 ? (
                <tr>
                  <td colSpan={8} className="px-3 py-10 text-center text-muted-foreground">조건에 맞는 주문이 없습니다.</td>
                </tr>
              ) : (
                orders.map((row) => (
                  <tr key={row.id} className="border-b border-border align-top hover:bg-muted/20">
                    ...
                    <td className="px-3 py-3 font-medium">{formatPrice(Number(row.payableAmount ?? row.totalPrice + row.shippingFee))}</td>
                    <td className="px-3 py-3 text-xs text-muted-foreground">
                      {row.promotionLabel ? `${row.promotionLabel} / -${formatPrice(Number(row.promotionDiscount ?? 0))}` : '-'}
                    </td>
                    ...
                    <td className="px-3 py-3">
                      <div className="flex flex-wrap gap-2">
                        ...
                        {row.status === 'paid' && row.shippingStatus === 'shipped' ? (
                          <Button size="sm" variant="outline" onClick={() => void changeShipping(row, 'delivered')} disabled={patchMutation.isPending}>
                            배송완료
                          </Button>
                        ) : null}
                      </div>
            ...
            <div className="space-y-5 px-5 py-5 text-sm">
              <section className="grid gap-3 border-b border-border pb-5 sm:grid-cols-2">
                <div>
                  ...
                  <p className="mt-1 font-mono">{detailOrder.displayOrderId ?? detailOrder.orderId}</p>
                </div>
                <div>
              ...
              <section className="space-y-2 border-b border-border pb-5">
                ...
                {detailOrder.shippingAddress?.detailAddress ? <p className="text-muted-foreground">{detailOrder.shippingAddress.detailAddress}</p> : null}
              ...
              <section className="space-y-3 border-b border-border pb-5">
                ...
                <p className="text-xs text-muted-foreground">배송중 처리 전 택배사와 송장번호가 올바르게 입력되었는지 확인해 주세요.</p>
              </section>

              <section className="space-y-3 border-b border-border pb-5">
                <h3 className="font-medium text-foreground">주문 품목</h3>
              ...
              {detailOrder.returnStatus === 'requested' ? (
                <section className="border border-destructive/40 bg-destructive/5 px-4 py-3">
                  ...
                  <p className="whitespace-pre-wrap text-muted-foreground">{detailOrder.returnReason || '사유 없음'}</p>
                </section>
              ) : null}

              {detailOrder.status === 'exchange_requested' ? (
                <section className="border border-blue-300 bg-blue-50/30 px-4 py-3">
                  <div className="mb-2 flex items-center justify-between gap-3">
                    <p className="font-medium text-blue-700">교환 요청</p>
                    <Button
                      size="sm"
                      className="bg-blue-600 text-white hover:bg-blue-700"
                      onClick={() => {
                        if (window.confirm('교환 완료 처리하시겠습니까?')) void completeExchange(detailOrder.id);
                      }}
                      disabled={patchMutation.isPending}
                    >
                      교환 완료 처리
                  ...
                  <p className="whitespace-pre-wrap text-muted-foreground">{detailOrder.exchangeReason || '사유 없음'}</p>
                </section>
              ) : null}
            </div>
          ) : null}
