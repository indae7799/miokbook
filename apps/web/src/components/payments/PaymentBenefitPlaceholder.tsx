...
export default function PaymentBenefitPlaceholder({
  ...
  return (
    <section
      className={`rounded-2xl border border-[#722f37]/12 bg-[linear-gradient(180deg,#fffdf9_0%,#f7efe6_100%)] ${
        compact ? 'p-4' : 'p-5'
      } shadow-[0_18px_40px_-34px_rgba(114,47,55,0.26)]`}
    >
      <div className="flex items-start justify-between gap-3">
        <div>
          ...
          <h3 className={`mt-1 font-semibold text-[#241916] ${compact ? 'text-[15px]' : 'text-[17px]'}`}>
            {title}
          </h3>
        </div>
      ...
      <div className={`mt-4 grid gap-2.5 ${compact ? '' : 'sm:grid-cols-2'}`}>
        <div className="rounded-xl bg-white/78 px-3.5 py-3">
          <p className="text-xs font-medium text-[#2e211d]">카드 즉시할인</p>
          <p className="mt-1 text-xs leading-5 text-[#6d5a52]">
      ...
      <p className="mt-3 text-[11px] leading-5 text-[#8e6f61]">
